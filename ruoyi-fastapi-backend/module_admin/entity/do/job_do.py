...
class SysJob(Base):
    ...
    job_group = Column(String(64), nullable=False, default='default', comment='任务组名')
    job_executor = Column(String(64), nullable=False, default='default', comment='任务执行器')
    invoke_target = Column(String(500), nullable=False, comment='调用目标字符串')
    job_args = Column(String(255), nullable=True, comment='位置参数')
    job_kwargs = Column(String(255), nullable=True, comment='关键字参数')
    cron_expression = Column(String(255), nullable=True, default='', comment='cron执行表达式')
    misfire_policy = Column(String(20), nullable=True, default='3', comment='计划执行错误策略（1立即执行 2执行一次 3放弃执行）')
